  ...
  computed: {
    ...
    filteredVideos() {
      ...
      return this.allVideos.filter(video => {
        // 难度筛选
        const levelMatch = !this.filterConditions.level || 
                          video.hard === this.filterConditions.level;
        
        // 类型筛选（多选）
        const typeMatch = this.filterConditions.types.length === 0 || 
                         this.filterConditions.types.some(type => 
                           video.type.includes(type));
